  ...
  switch (host.sld) {
    ...
    case "youtu":
      if (url.hostname === "youtu.be" && parts.length >= 2) {
        ...
        url = new URL(
          `https://youtube.com/watch?v=${encodeURIComponent(parts[1])}`
        );
      }
      break;
    ...
    case "pin":
      if (url.hostname === "pin.it" && parts.length === 2) {
        url = new URL(
          `https://pinterest.com/url_shortener/${encodeURIComponent(parts[1])}`
        );
      }
      break;
    ...
    case "fb":
      if (url.searchParams.get("v")) {
        url = new URL(
          `https://web.facebook.com/user/videos/${url.searchParams.get("v")}`
        );
      }
    ...
    case "ddinstagram":
      if (
        services.instagram.altDomains.includes(host.domain) &&
        [null, "d", "g"].includes(host.subdomain)
      ) {
    ...
    case "linkedin":
      if (parts[1] === "posts") {
        ...
        url = new URL(
          `https://linkedin.com/feed/update/urn:li:activity:${postId}`
        );
      }
      break;
  }
...
function getHostIfValid(url) {
  ...
  const validSubdomain = [null, "www", ...(service.subdomains ?? [])].includes(
    host.subdomain
  );
...
export function normalizeURL(url) {
  return cleanURL(aliasURL(new URL(url.replace(/^https\/\//, "https://"))));
}
...
export function extract(url) {
  ...
  for (const pattern of services[host].patterns) {
    patternMatch = pattern.match(url.pathname.substring(1) + url.search);
