export const testers = {
  bilibili: (patternMatch) =>
    patternMatch.comId?.length <= 12 ||
    patternMatch.comShortLink?.length <= 16 ||
  ...
  loom: (patternMatch) => patternMatch.id?.length <= 32,

  ok: (patternMatch) => patternMatch.id?.length <= 16,
  ...
  snapchat: (patternMatch) =>
    (patternMatch.username?.length <= 32 &&
      (!patternMatch.storyId || patternMatch.storyId?.length <= 255)) ||
    patternMatch.spotlightId?.length <= 255 ||
    patternMatch.shortLink?.length <= 16,

  streamable: (patternMatch) => patternMatch.id?.length === 6,
  ...
  linkedin: (patternMatch) => patternMatch.id?.length === 19,
};
